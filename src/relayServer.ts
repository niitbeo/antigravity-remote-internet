...
export class RelayServer {
    ...
    getConnectUrls(): string[] {
        ...
        const urls = ips.map((ip) => `http://${ip}:${this.port}/?token=${this.token}`);
        const custom = this.buildPublicUrl();
        if (custom) {
            urls.unshift(custom);
        }
        return urls;
    ...
    private buildPublicUrl(): string | null {
        const raw = (this.publicBaseUrl ?? '').trim();
        if (!raw) {
            return null;
        }
