...
class RemoteController implements vscode.Disposable {
    ...
    async start(): Promise<void> {
        ...
        if (publicBaseUrl) {
            this.output.appendLine(`Public URL configured: ${publicBaseUrl}`);
            this.relay.addActivity(`Internet mode URL enabled: ${publicBaseUrl}`);
        }
...
<html>
  <body style="font-family: sans-serif; padding: 16px;">
    ...
    private createRelay(host: string, port: number, token: string, publicBaseUrl?: string): RelayServer {
